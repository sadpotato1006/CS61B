...
* 文件 --(java.Util.readContents)>> 文件内容(byte[])
* 文件内容(byte[])--(sha1)>>文件哈希值
* **!!! error: ```sha1(serialize(readContents(f)))``` readContent得到的已经是byte[],不能再用serialize。**
true: ```sha1(readContents(f))```
* 文件名 --(staged)>> 文件哈希值
* 文件名 --(removed)>> 文件哈希值
* 文件哈希值 --(blobs)>> 文件名
